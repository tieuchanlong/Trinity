...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, LPWSTR cmdLine, int cmdShow)
{
	...
	// Register game classes first
	GameApplication::GetInstance()->RegisterGameClass();

	// Do some application initialization for application, Direct3D
	if (GameApplication::GetInstance()->Initialize(hInstance, WndProc, cmdShow))
	{
	...
	int iRet = GameApplication::GetInstance()->Run();

	GameApplication::GetInstance()->Destroy();
